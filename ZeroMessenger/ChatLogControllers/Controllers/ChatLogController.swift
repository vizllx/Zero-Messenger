...
class ChatLogController: UICollectionViewController {
  ...
  func configureTitleViewWithOnlineStatus() {
       DispatchQueue.main.async {  [unowned self] in
        if let isGroupChat = self.conversation?.isGroupChat, isGroupChat, let title = self.conversation?.chatName,
            let membersCount = self.conversation?.chatParticipantsIDs?.count {
      let subtitle = "\(membersCount) members"
      self.navigationItem.setTitle(title: title, subtitle: subtitle)
      return
    }

        guard let currentUserID = Auth.auth().currentUser?.uid, let toId = self.conversation?.chatID else { return }
    ...
    if currentUserID == toId {
      ...
      return
        ...
        self.userStatusReference = Database.database().reference().child("users").child(toId)
        self.userHandler = self.userStatusReference.observe(.value, with: { (snapshot) in
