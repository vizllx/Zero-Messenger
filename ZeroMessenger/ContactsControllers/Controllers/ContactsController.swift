...
var globalUsers: [User] = [] {
  didSet {
    NotificationCenter.default.post(name: .zeroUsersUpdated, object: nil)
  }
}

private let zeroUsersCellID = "zeroUsersCellID"
...
class ContactsController: UITableViewController {
  ...
  let zeroUsersFetcher = ZeroUsersFetcher()
  let contactsFetcher = ContactsFetcher()
    var countFetchUser  = 0
    ...
    override func viewDidLoad() {
      ...
      addObservers()
        ...
        if let app = UIApplication.shared.delegate as? AppDelegate, let window = app.window?.rootViewController?.view {
             ARSLineProgress.ars_showOnView(window)
        }
        DispatchQueue.main.asyncAfter(deadline: .now() + 2.0) {  self.contactsFetcher.fetchContacts()
        }
    ...
    override func viewWillAppear(_ animated: Bool) {
      ...
      guard shouldReFetchZeroUsers else { return }
      shouldReFetchZeroUsers = false
       
      DispatchQueue.main.async {  [unowned self] in
        self.zeroUsersFetcher.fetchZeroUsers(asynchronously: true)
      }
    }
    ...
    fileprivate func configureViewController() {
      zeroUsersFetcher.delegate = self
      ...
      tableView.register(ZeroUsersTableViewCell.self, forCellReuseIdentifier: zeroUsersCellID)
    ...
    fileprivate func reloadTableView(updatedUsers: [User]) {
      ...
      self.users = zeroUsersFetcher.rearrangeUsers(users: self.users)
    ...
    func selectCell(for indexPath: IndexPath) -> UITableViewCell {
      if indexPath.section == 0 {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: zeroUsersCellID,
                                                 for: indexPath) as? ZeroUsersTableViewCell ?? ZeroUsersTableViewCell()
...
extension ContactsController: ZeroUsersUpdatesDelegate {
...
//        if self.countFetchUser == 0 {
//
//            ARSLineProgress.ars_showOnView(self.view)
//            shouldReFetchZeroUsers = true
//            self.countFetchUser = self.countFetchUser + 1
//           self.zeroUsersFetcher.fetchZeroUsers(asynchronously: true)
//
//
//    }
  ...
  func contacts(updateDatasource contacts: [CNContact]) {
    ...
    DispatchQueue.main.async { [unowned self] in
     ...
     // ARSLineProgress.hide()
      ...
      self.zeroUsersFetcher.fetchZeroUsers(asynchronously: false)
    }
