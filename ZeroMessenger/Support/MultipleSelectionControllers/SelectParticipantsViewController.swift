...
//  ZeroMessger
//
//  Created by Sandeep Mukherjee on 3/6/20.
...
class SelectParticipantsViewController: UIViewController {
  ...
  func createGroup() {
    ...
    destination.selectedFlaconUsers = selectedZeroUsers
  ...
  func addNewMembers() {
    ...
    reference.observeSingleEvent(of: .value) { (snapshot) in
      ...
      for selectedUser in self.selectedZeroUsers {
  ...
  fileprivate func setupTableView() {
    ...
    tableView.register(ParticipantTableViewCell.self, forCellReuseIdentifier: zeroUsersCellID)
  ...
  func didSelectUser(at indexPath: IndexPath) {
    ...
    selectedZeroUsers.append(sections[indexPath.section][indexPath.row])
  ...
  func didDeselectUser(at indexPath: IndexPath) {
    ...
    let user = sections[indexPath.section][indexPath.row]
      ...
      users[index].isSelected = false
		...
		if let selectedZeroUserIndexInCollectionView = selectedZeroUsers.firstIndex(of: user) {
      selectedZeroUsers[selectedZeroUserIndexInCollectionView].isSelected = false
      selectedZeroUsers.remove(at: selectedZeroUserIndexInCollectionView)
      DispatchQueue.main.async {
         self.reloadCollectionView()
      }
    }
