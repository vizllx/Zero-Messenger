...
//  ZeroMessger
//
//  Created by Sandeep Mukherjee on 3/12/20.
...
extension SelectParticipantsViewController: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
  ...
  func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
    return selectedZeroUsers.count
  }
  ...
  func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
    ...
    cell.title.text = selectedZeroUsers[indexPath.item].name
  ...
  func selectSize(indexPath: IndexPath) -> CGSize  {
    ...
    guard let userName = selectedZeroUsers[indexPath.row].name else { return  CGSize(width: 100, height: cellHeight) }
