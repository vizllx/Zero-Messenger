...
//  ZeroMessger
//
//  Created by Sandeep Mukherjee on 3/27/20.
...
class SelectNewAdminTableViewController: UITableViewController {
  ...
  fileprivate func setupTableView() {
    ...
    tableView.register(NewAdminTableViewCell.self, forCellReuseIdentifier: zeroUsersCellID)
  ...
  func setNewAdmin(membersIDs: [String]) {
    guard let newAdminID = selectedZeroUsers[0].id, let newAdminName = selectedZeroUsers[0].name else { return }
  ...
  func selectCell(for indexPath: IndexPath) -> UITableViewCell? {
    ...
    let cell = tableView.dequeueReusableCell(withIdentifier: zeroUsersCellID, for: indexPath) as? NewAdminTableViewCell ?? NewAdminTableViewCell()
    ...
    if let statusString = sections[indexPath.section][indexPath.row].onlineStatus as? String {
      if statusString == statusOnline {
        cell.subtitle.textColor = ZeroPalette.defaultBlue
