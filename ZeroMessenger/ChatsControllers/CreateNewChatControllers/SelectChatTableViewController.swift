...
extension NSNotification.Name {
  static let zeroUsersUpdated = NSNotification.Name(Bundle.main.bundleIdentifier! + ".zeroUsers")
}

class SelectChatTableViewController: UITableViewController {
  
  let zeroUsersCellID = "zeroUsersCellID"
  ...
  fileprivate func addObservers() {
    NotificationCenter.default.addObserver(self, selector: #selector(updateUsers), name: .zeroUsersUpdated, object: nil)
  }
  ...
  func selectCell(for indexPath: IndexPath) -> UITableViewCell? {
    ...
    if indexPath.section == headerSection {
      ...
      cell.textLabel?.textColor = ZeroPalette.defaultBlue
      return cell
    }
    ...
    let zeroUser = filteredUsersWithSection[indexPath.section][indexPath.row]
    cell.configureCell(for: zeroUser)
    return cell
  }
  ...
  override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
    ...
    if indexPath.section == 0 {
     ...
     // destination.setUpCollation()
      ...
      let conversationDictionary: [String: AnyObject] = ["chatID": zeroUser.id as AnyObject, "chatName": zeroUser.name as AnyObject,
                                                         "isGroupChat": false  as AnyObject,
                                                         "chatOriginalPhotoURL": zeroUser.photoURL as AnyObject,
                                                         "chatThumbnailPhotoURL": zeroUser.thumbnailPhotoURL as AnyObject,
                                                         "chatParticipantsIDs": [zeroUser.id, currentUserID] as AnyObject]
